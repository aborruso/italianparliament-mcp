...
| `senato-votes list` | Votazioni d'Assemblea del Senato con esito, contatori (favorevoli/contrari/astenuti), tipo, data e DDL collegato. Filtrabile per legislatura, data, DDL, parola chiave nel label (`--keyword`), voti di fiducia (`--confidence-vote`) e voti finali (`--final-vote`) |
...
| `senato-attendance show` | Conteggio aggregato dei voti di un senatore in tutte le votazioni d'Assemblea di una legislatura (favorevole/contrario/astenuto/presente non votante/in congedo o missione) — misura di attivismo/assenteismo. Richiede `--legislature` (l'URI del senatore non la contiene) |
