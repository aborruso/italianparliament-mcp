...
Fix: oltre alla risoluzione via link forte, la risoluzione delle date ora interroga anche le fiducie della legislatura (label contiene "fiducia", non "sfiducia") il cui numero estratto dal label (`extractBillNumber`, lo stesso helper del Fallback 1) corrisponde alla `osr:fase` del DDL cercato — indipendentemente dalla data della seduta. Verificato: il decreto sicurezza ora restituisce sia pregiudiziale (3/6) sia fiducia (4/6); nessuna regressione sui casi same-day (Piano Casa, DDL 1561) o storici (leg.18, DDL 52988). Regressione coperta dal test `senato-votes: --ddl-uri includes the fiducia when it falls on a different seduta than la strongly-linked vote`.

**Residuo noto**: il match è per numero **esatto**, suffisso di lettura incluso (`osr:fase` = "S.562-B" richiede che il label citi "562-B", non solo "562"). Se una fiducia cita il numero base ma il DDL cercato è una lettura successiva (`-B`, `-bis`), il match fallisce e la fiducia resta fuori — stesso tipo di gap del refuso in "Recupero di `bill_number`" sopra, non recuperabile da questo fallback. Scelta deliberata: allargare il match al numero base (`billBaseNumber`) rischierebbe di agganciare la fiducia alla lettura **sbagliata** quando lo stesso numero base ha più letture con voti di fiducia distinti — un falso positivo peggiore del falso negativo attuale. Nessun caso reale osservato finora nei test-sentinella; da riaprire se emerge un caso concreto.
